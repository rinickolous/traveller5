...
const symlinkPath = path.resolve(dataPath, "systems", "traveller5");
...
if (symlinkStats) {
	...
	const proceed: boolean = (
		await prompts({
			...
			name: "value",
			initial: false,
			message: `A "traveller5" ${atPath} already exists in the "systems" subfolder. Replace with new symlink?`,
		})
	).value;
	if (!proceed) {
