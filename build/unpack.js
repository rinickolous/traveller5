...
for (const pack of packs) {
	...
	await extractPack(`${SYSTEM_ID}/dist/packs/${pack}`, `${SYSTEM_ID}/src/packs/${pack}`, {
		yaml,
		transformName,
		transformEntry,
		expandAdventures,
...
async function transformEntry(entry) {
	...
	// Remove module flags
	for (const key of Object.keys(entry.flags)) if (!["core", "traveller5"].includes(key)) delete entry.flags[key];
	...
	for (const embeddedCollection of ["items", "effects", "pages"]) {
		if (entry[embeddedCollection]) {
			for (const e of entry[embeddedCollection]) {
				...
				if (e["effects"])
					for (const grandchild of e["effects"])
						Object.assign(grandchild, { modifiedTime: null, lastModifiedBy: null });
			}
		}
	}
	...
	for (const jep of entry.pages) {
		const docsPath = path.join("src", "docs", jep.flags["traveller5"].wikiPath);
